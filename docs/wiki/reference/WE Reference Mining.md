...
Renderer-state recovery pins the light type IDs to point=0, spot=1, tube=2,
directional=3. Defaults are exponent=2, innercone=20°, outercone=30°, and tube
controlpoint=(2,0,0). Spot thresholds are `cos(radians(angle))` (no half-angle):
inner goes in `Origin.w`, outer in `Direction.w`. Tube endpoint B is the full
world-matrix transform of the local control point with w=1.
...
As of 2026-07-17, the fork implements this at a high level with a comparison-
depth atlas: one projection for a spot light, three stable cascades for a
directional light, and six faces for a point light. Static and skinned opaque
`CModel` submeshes cast shadows. Alpha-cutout/translucent casters, non-model
casters, and exact native bias/filter parity remain open.
...
## Global uniform inventory

Fog is implemented through `g_FogDistanceColor/Params` and
`g_FogHeightColor/Params` (scene keys `fogdistance*`/`fogheight*`: start/end +
startdensity/enddensity). `g_Bones` is now populated for supported skinned 3D
models. Still unset: HDR `g_HDRParams`; view basis
`g_ViewForward/Right/Up`,
`g_OrientationForward/Right/Up`. Remaining skinning/morph globals include
`g_BonesAlpha` and `g_MorphOffsets/Weights/BoneRules/BoneTransform` (morph data via
...
- The machine-local command workflow and reusable headless Ghidra scripts live
  in `.claude/lwe-terminal-runbook.md` and `.claude/tools/ghidra/`. Keep
  absolute addresses tied to the executable hash because updates move them.
