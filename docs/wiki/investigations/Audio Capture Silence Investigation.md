...
description: Historical resolved symptom — July evidence showed PulseAudio-layer monitor capture silent while native PipeWire capture worked.
...
# Audio Capture Silence Investigation (CURRENTLY RESOLVED)

> **Current status (2026-07-17):** the user confirms that audio-reactive
> wallpapers now respond normally while music plays. The flat-capture symptom
> is closed. The exact 64→32/16 spectrum reduction and full native DSP parity
> remain separate open verification items.
...
**Historical status:** root cause was not found on July 8. At that time the
engine appeared to be a *victim*, not the culprit:
...
At the time of investigation, any capture client going through the
**PulseAudio shim** (`pipewire-pulse`) and
...
5. **Historical windowed test with `WPE_AUDIO_DEBUG=1` (9 s):** the older
   gated recorder produced no debug log while silent. The current recorder no
   longer has that gate and logs RMS/bands for every periodic completed capture
   block, including zero-valued blocks.
...
# Engine-side view: current code logs RMS/bands periodically, including zeroes
WPE_AUDIO_DEBUG=1 ./build-new/output/linux-wallpaperengine --window 100x100x640x360 \
  -bg ~/.steam/root/steamapps/workshop/content/431960/3094637759 --noautomute
tail -f /tmp/we-audio-debug.log
```
