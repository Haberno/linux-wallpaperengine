...
class ScriptTime(unittest.TestCase):
    def test_frametime_limits_long_frames_before_authored_fixed_step_catchup(self):
...
export function update(value) {
    ...
    if (frame === 3) {
        stalledAt = Date.now();
        while (Date.now() - stalledAt < 500) {}
    }
    if (frame === 4) {
        let accumulator = engine.frametime;
        let steps = 0;
        while (accumulator >= 1 / 96 && steps < 96) {
            accumulator -= 1 / 96;
            steps++;
        }
        console.log('STALL_CLOCK ' + JSON.stringify({
            dt: engine.frametime, elapsedMs: Date.now() - stalledAt, steps
        }));
    }
    return value;
}
'''
        with tempfile.TemporaryDirectory(prefix='lwe-clock-stall-') as directory:
            _, output = render_scene(self, Path(directory), {
        ...
        samples = [json.loads(row.split('STALL_CLOCK ', 1)[1])
                   for row in output.splitlines() if 'STALL_CLOCK ' in row]
        self.assertEqual(len(samples), 1, output)
        self.assertGreaterEqual(samples[0]['elapsedMs'], 500)
        # The reference runtime reports exactly .25 after a 1.5-second stall.
        self.assertEqual(samples[0]['dt'], .25)
        self.assertEqual(samples[0]['steps'], 24)
