...
class EffectAnimation(unittest.TestCase):
    def test_unnamed_current_property_timeline_starts_from_media_hook(self):
        script = """
export function mediaThumbnailChanged() {
    const fade = thisObject.getAnimation();
    if (!fade || fade.isPlaying()) throw Error('default property animation unavailable');
    fade.play();
    console.log('DEFAULT_EFFECT_ANIMATION_OK');
}
"""
        with tempfile.TemporaryDirectory(prefix='lwe-effect-default-') as directory:
            root = Path(directory)
...
void main() { gl_FragColor = vec4(uAmount, 0, 0, 1.0); }
''')
            ...
            timeline = animation(0, 1, startpaused=True)
            timeline['options'].pop('name', None)
            scene = base_scene([{'id': 1, 'name': 'Fade', 'image': 'models/probe.json',
                'origin': '160 90 0', 'size': '100 100', 'effects': [{
                    'id': 2, 'file': 'effects/probe.json', 'passes': [{
                        'constantshadervalues': {'amount': {
                            'value': 0, 'script': script, 'animation': timeline}}}]}]}])
            scene['general']['camerafade'] = False
            frame, output = render_scene(self, root, scene, frames=15)
            self.assertIn('DEFAULT_EFFECT_ANIMATION_OK', output)
            self.assertGreater(frame.getpixel((160, 90))[0], 250)
